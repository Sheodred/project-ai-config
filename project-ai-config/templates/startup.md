...
4. **Handoff doc.** Handoffs live in `~/.claude/handoff/<project>/` (`~` = your home directory: `/home/<user>` on Linux, `/Users/<user>` on macOS, `C:\Users\<user>` on Windows). One flat file per handoff named `<YYYY-MM-DD>T<HHMM>-<slug>.md`. The timestamp prefix sorts newest-last within a project's folder.
   - If this session is clearly associated with this repo, take the newest file in `~/.claude/handoff/<PROJECT>/`, read it, and print the full content so both of you have it in view before continuing.
